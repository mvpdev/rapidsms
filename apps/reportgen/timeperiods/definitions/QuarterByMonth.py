...
class QuarterByMonth(PeriodType):

    title       = _("Quarter (by Month)")
    description = _("Quarter starting X months ago")
    code        = 'QM'
    n_periods   = 24

    @classmethod
    def periods(cls):
        return [cls._quarter_period(index) \
            for index in xrange(0, cls.n_periods)]

    @classmethod
    def _quarter_period(cls, index):
        # Index == 0 means starting this month
        # Index == 1 means starting last month

        # If we are in March, Quarter starts January
        # e.g., We go from January 1, 2010 to March 31, 2011

        # First day of next month, starting three months ago

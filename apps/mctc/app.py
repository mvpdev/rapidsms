    ...
    def new_case_frccsn (self, message, token_string):
        ...
        if len(dob) >= 3:
            try:
                ...
                try:
                    ...
                    raise HandlerFailed(_("Ne comprend pas la date: %s") % dob)
                    #raise HandlerFailed(_("Couldn't understand date: %s") % dob)
            self.debug(dob)
        ...
        if location_code == "":
            ...
            try:
                ...
                message.respond(_(u"Ne peux pas figurer hors de la ou vous etes aujourd hui. SVP utiliser le message: Location locationcode pour enregistrer la location."))
                return True
        else:
            try:
                location    = Location.objects.get(code=location_code)
                ...
                message.respond(_(u"Ne trouve pas la location correspondant au code que vous avez envoye. SVP renvoyez."))
                return True
        ...
        if iscase:
            ...
            #"%(last_name)s, %(first_name)s (+%(PID)s) has already been registered by %(reporter)s.") % info)
            message.respond(_(
            "%(last_name)s, %(first_name)s (+%(PID)s) a deja ete enregistre par %(reporter)s.") % info)

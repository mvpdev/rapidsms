...
def test():
    ...
    try:
        ...
        print "You can add a django model as a state then save the state before"\
        " the tracked item"
        t1.state = l[0]
        t1.save()
        s1 = t1.state
        ...
        print "You can't cancel the current state is it's not saved"
        t1.state = p[22]
        try:
            del t1.state
            assert False
        ...
        print "You can't save states outside a TrackedItem"

...
class ComparisonNode(template.Node):
    ...
    def render(self, context):
        try:
            if COMPARISON_DICT[self.comparison](self.var1.resolve(context), self.var2.resolve(context)):                
...
def do_comparison(parser, token):
    ...
    Examples::
        ...
        {% if_in user in user_list %}
        <p>{{ user }} is a member of user_list.</p>
        {% endif_in %}    
...
for tag_name in ('if_less', 'if_less_or_equal', 'if_greater_or_equal', 'if_greater', 'if_in', 'if_not_in'):

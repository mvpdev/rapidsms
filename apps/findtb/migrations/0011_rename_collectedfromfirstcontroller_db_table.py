...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        db.rename_table('findtb_collectedfromfirstcontroller', 'findtb_collectedfrom1stctrl')
        if not db.dry_run:
            try:
                ct = orm['contenttypes.ContentType'].objects.get(model='collectedfromfirstcontroller')
            except orm['contenttypes.ContentType'].DoesNotExist:
                pass
            else:
                ct.model = 'collectedfrom1stctrl'
                ct.save()

    def backwards(self, orm):
        db.rename_table('findtb_collectedfrom1stctrl', 'findtb_collectedfromfirstcontroller')
        if not db.dry_run:
            try:
                ct = orm['contenttypes.ContentType'].objects.get(model='collectedfrom1stctrl')
            except orm['contenttypes.ContentType'].DoesNotExist:
                pass
            else:
                ct.model = 'collectedfromfirstcontroller'
                ct.save()

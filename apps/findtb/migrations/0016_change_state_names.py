...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        self.set_registered_titles(orm, 'registered')
        self.set_sent_titles(orm, 'sent')

    def backwards(self, orm):
        self.set_registered_titles(orm, 'incoming')
        self.set_sent_titles(orm, 'incoming')

    def set_registered_titles(self, orm, title):
        registered_ct = orm['contenttypes.ContentType'].objects.get(model='specimenregistered')
        late_sending_ct = orm['contenttypes.ContentType'].objects.get(model='sendingislate')

        for state in orm['django_tracking.State'].objects.filter(Q(content_type=registered_ct) | Q(content_type=late_sending_ct)):
            state.title = title
            state.save()

    def set_sent_titles(self, orm, title):
        sent_ct = orm['contenttypes.ContentType'].objects.get(model='specimensent')
        late_delivery_ct = orm['contenttypes.ContentType'].objects.get(model='deliveryislate')

        for state in orm['django_tracking.State'].objects.filter(Q(content_type=sent_ct) | Q(content_type=late_delivery_ct)):
            state.title = title
            state.save()
    ...
    models = {
        ...
        'findtb.sendingislate': {
            'Meta': {'object_name': 'SendingIsLate', '_ormbases': ['findtb.SpecimenRegistered']},
            'specimenregistered_ptr': ('django.db.models.fields.related.OneToOneField', [], {'to': "orm['findtb.SpecimenRegistered']", 'unique': 'True', 'primary_key': 'True'})
        },
        'findtb.sirezresult': {

...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        db.rename_table('findtb_deliveredtosecondcontroller', 'findtb_deliveredto2ndctrl')
        if not db.dry_run:
            try:
                ct = orm['contenttypes.ContentType'].objects.get(model='deliveredtosecondcontroller')
            except orm['contenttypes.ContentType'].DoesNotExist:
                pass
            else:
                ct.model = 'deliveredto2ndctrl'
                ct.save()

    def backwards(self, orm):
        db.rename_table('findtb_deliveredto2ndctrl', 'findtb_deliveredtosecondcontroller')
        if not db.dry_run:
            try:
                ct = orm['contenttypes.ContentType'].objects.get(model='deliveredto2ndctrl')
            except orm['contenttypes.ContentType'].DoesNotExist:
                pass
            else:
                ct.model = 'deliveredtosecondcontroller'
                ct.save()

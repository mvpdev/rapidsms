    ...
    def process(self, patient):
        if len(self.params) < 4:
            raise ParseError(_(u"Not enough info. Expected:  " \
                                " | number of secondary school aged pupils in " \
                                "the household" \
        ...
        if not self.params[2].isdigit():
            raise ParseError(_(u"| Number of secondary school aged pupils  " \
                                "atttending secondary school must be entered " \
                                "as a number."))
        ...
        psa.school_type = psa.SECONDARY_SCHOOL
        ...
        if len(self.params) < 5:
            raise ParseError(_(u"After entering number of under/over school  " \
                                "aged pupils attending secondary school, you  " \
        ...
        if unkown:
            invalid_str = _(u"Unkown Reason for not attending school " \
                              "code(s): %(codes)s " \
                          "No secondary school attendance recorded.") % \
